...
class ResearchOutput
{
    ...
    public function getPublicationList($settings)
    {
        ...
        if ($settings['wFlowSteps_forRevalidation'] == 1) {
            $xml .= "<workflowSteps>forRevalidation</workflowSteps>";
        }
        if ($settings['wFlowSteps_validated'] == 1) {
            $xml .= "<workflowSteps>validated</workflowSteps>";
        }
        if ($settings['wFlowSteps_approved'] == 1) {
            $xml .= "<workflowSteps>approved</workflowSteps>";
        }
        if ($settings['wFlowSteps_forValidation'] == 1) {
            $xml .= "<workflowSteps>forValidation</workflowSteps>";
        }
        if ($settings['wFlowSteps_forRevalidation'] == 1) {
            $xml .= "<workflowSteps>forRevalidation</workflowSteps>";
        }
    ...
    public function transformArray($publications, $settings)
    {
        ...
        foreach ($publications['items'] as $contribution) {
            ...
            if (($settings['onlypublicationStatusPublished'] == 1) && ($array['contributionToJournal'][$i]['publicationStatus']['uri'] != '/dk/atira/pure/researchoutput/status/published')) {
                unset($array['contributionToJournal'][$i]);
            } else {
                $i++;
            }
        }
